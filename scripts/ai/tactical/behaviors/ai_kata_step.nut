this.ai_kata_step <- this.inherit("scripts/ai/tactical/behavior", {
	...
	function onEvaluate( _entity )
	{
		...
		for( local i = 0; i < 6; i++ )
		{
			if (myTile.hasNextTile(i))
			{
				...
				if (nextTile.IsEmpty && this.m.Skill.isUsableOn(nextTile))
				{
		...
		// TODO: If we are not in zone of control, then always use this skill to
		// move, provided that we really want to move in there.
		/* if (!inZonesOfControl)
		{
			return this.Const.AI.Behavior.Score.KataStep * scoreMult;
		} */
	...
	function onExecute( _entity )
	{
		if (this.m.IsFirstExecuted)
		{
			...
			{
				this.logInfo("* " + _entity.getName() + ": using Kata Step!");
			}
